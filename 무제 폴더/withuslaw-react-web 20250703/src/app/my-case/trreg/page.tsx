...
export default function MY_TR_001_M() {
  ...
  const onSubmit = (data: TForm) => {
    ...
    if (onlyNumValue?.startsWith("9")) {
      // Toast 표시
      callToast({
        msg: "유효하지 않은 등기고유번호입니다.\n등기고유번호 수정 후 다시 시도해주세요.",
        status: "notice",
      });

      // 에러 표시
      setError("faRgstrUnqNo1", {
        type: "manual",
        message: "등기 고유번호가 올바르지 않습니다.",
      });
      return;
    }
            ...
            <Input.InputContainer className="px-4 py-4 pb-6">
              ...
              <Controller
                ...
                rules={{
                  required: false,
                  // validate: (value) => {
                  //   const onlyNum = value?.replace(/\D/g, ""); // 숫자만 추출
                  //   /* 2025.06.27 등기고유번호가 "9"로 시작하면 토스트팝업 노출 및 에러표시 */
                  //   // if (onlyNum?.startsWith("9")) {
                  //   //   callToast({
                  //   //     msg: "등기 고유번호를 확인해 주세요.",
                  //   //     status: "notice",
                  //   //   });
                  //   // }
                  //   return !onlyNum?.startsWith("9");
                  // },
                }}
                render={({ field: { onChange, onBlur, value } }) => {
                  ...
                  return (
                    <Input.InputField
                      styleType={errors.faRgstrUnqNo1 ? "error" : "default"}
                      inputType="text"
