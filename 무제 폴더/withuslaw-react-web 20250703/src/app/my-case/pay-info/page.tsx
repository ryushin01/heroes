...
export default function MY_PI_002M() {
  ...
  const [isRpyAcctLoading, rpyAcctData] = useSearchRpyAcct();
  ...
  const {
    isOpen: isSellerPayOverbillion,
    close: closeSellerPayOverbillion,
    open: openSellerPayOverbillion,
  } = useDisclosure();
  const { isOpen: isOpenB200Error, open: openB200Error } = useDisclosure();
  const [b200ErrorMessage, setB200ErrorMessage] = useState("");
  const [checkOverBillon, setCheckOverBillon] = useState(false);
  ...
  useEffect(() => {
    if (checkOverBillon) {
      onSubmitForm(); //onSubmitForm 재실행
      setCheckOverBillon(false); //checkOverBillon false로 초기화
      closeSellerPayOverbillion(); // 매도인 10억 안내 팝업 닫기
    }
  }, [checkOverBillon]);
  ...
  function onSubmitForm() {
    ...
    /* 매도인 지급금이 10억 이상인지 체크 */
    const isOverSellerPayAmtBillon =
      moneyStringToNumber(getValues().sellerPayAmt) >= 1000000000;

    /* 매도인 지급금이 10억 이상이고 checkOverBillon 이 false이면 sheet 닫고 팝업 실행 */
    if (isOverSellerPayAmtBillon && !checkOverBillon) {
      close();
      openSellerPayOverbillion();
      return; // early return
    }
  ...
  function maskAccountNo(acctNo: string): string {
    ...
    const maskedLength = acctNo.length - visibleLength;
    const maskedPart = "*".repeat(maskedLength > 0 ? maskedLength : 0);
    const visiblePart = acctNo.slice(-visibleLength);
    ...
    <>
      ...
      >
        ...
        <Alert
          ...
          bodyText={
            rpyAcctData?.woori
              ? `당행계좌(${rpyAcctData.woori.bankNm.replace(
                  "은행",
                  ""
                )} ${maskAccountNo(
                  rpyAcctData.woori.acctNo
                )})로 수정하거나 영업점 별도 처리해주세요.`
              : "당행계좌로 수정하거나 영업점 별도 처리해주세요."
          }
        />
        {/* 매도인 지급 금액이 10억 이상일때 */}
        <Alert
          isOpen={isSellerPayOverbillion}
          title={"매도인 지급 계좌가 우리은행 계좌인지 확인해주세요"}
          confirmText={"확인"}
          confirmCallBack={() => {
            setCheckOverBillon(true); //checkOverBillon true
          }}
          bodyText={
            "매도인 지급금이 10억 원 이상일 경우, 우리은행 외 계좌로는 대출금 지급이 제한됩니다. 영업점에 계좌 정보를 확인해주세요."
          }
        />
        {/* B200전문 응답 오류 시 */}
        <Alert
