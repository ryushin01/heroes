...
export default function ImagePage() {
  ...
  const { data: kcbResultData, mutate: getKcbResult } = useMutation({
    ...
    onSuccess(res) {
      if (
        ...
        /* 2025.06.24 path 수정 */
        setTimeout(() => {
          router.push(`/view/cover/${membList && membList.loanNo}000`);
        }, 1500);
      }
    },
  });
  ...
  /* 2025.06.24 res.code 체크 조건 추가 */
  // 이미지 저장 API
  const {
    data: test,
    mutate: saveImg,
    isPending: saveImagePending,
  } = useMutation({
    ...
    onSuccess(res) {
      if (res.code === "00") {
        attcFilCd = res?.data?.attcFilCd;
        saveImgData = res?.data?.saveImgData;
        ...
        if (attcFilCd === "B" || attcFilCd === "C") {
          ...
          if (isOnlyA) {
            tryRedirect(); // A 단독일 때만 이동
          }
        }
      } else {
        callToast({
          msg: res.msg?.trim() ? res.msg : "이미지 등록이 실패하였습니다.",
          status: "error",
        });
      }
    },
  ...
  // 관리자 요청 API
  const { mutate: adminReq, isPending: adminRequestPending } = useMutation({
  ...
  // 제출 대상 계산
  const submittedAttcFilCds = useMemo(() => {
    const set = new Set<string>();

    if (estbsData?.rrcpCnfmReqYn === "Y" && estbsData?.rrcpSbmtYn === "N") {
      set.add("A");
    }

    if (estbsData?.mvhhdCnfmReqYn === "Y" && estbsData?.mvhhdSbmtYn === "N") {
      set.add("B");
    }
  ...
  // A 단독 제출 여부 체크
  const isOnlyA =
    submittedAttcFilCds.size === 1 && submittedAttcFilCds.has("A");

  // 중복 라우팅 방지
  const alreadyRoutedRef = useRef(false);

  function tryRedirect() {
    if (!alreadyRoutedRef.current) {
      alreadyRoutedRef.current = true;
      router.push(`/view/cover/${membList.loanNo}000`);
    }
  }
  ...
  return (
    <>
      {(saveImagePending || adminRequestPending) && <Loading />}
      <WooriHeader />
      <form
