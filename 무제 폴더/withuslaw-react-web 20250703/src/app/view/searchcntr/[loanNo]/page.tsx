...
export default function Page() {
  ...
  if (typeof loanNoParams.loanNo === "string") {
    ...
    params.append("loanNo", `${loanNo}000`);
  }
  ...
  const { data: membData, isSuccess } = useQuery({
    ...
    queryFn: (): Promise<any> =>
      fetch(
        `${process.env.NEXT_PUBLIC_APP_WOORI_API_URL}/api/view/searchcntr/${loanNo}`,
        {
          method: "get",
          headers: {
  ...
  const { data: kcbDataList, mutate } = useMutation({
    ...
    mutationFn: (): Promise<TKcb> =>
      fetch(
        `${process.env.NEXT_PUBLIC_APP_KOS_API_URL}/api/auth/searchpopupkcbreq`,
        {
          method: "post",
          headers: {
            "Content-Type": "application/json",
