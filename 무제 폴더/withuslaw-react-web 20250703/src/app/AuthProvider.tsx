...
function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const { data, refetch, isLoading } = useQuery({
    ...
    queryFn: (): Promise<TAuth> =>
      fetch(
        ...
        }&pwd=${pinNo}&fcmId=000000000000&dvceUnqNum=System&method=BIO`,
        {
          method: "get",
          headers: {
  ...
  /**
   * 1. mount 후 3초 뒤에 레이어 페이드 아웃 타이머 작동
   * 2. 레이어 숨김 처리
   * 3. unmount 시 clean-up 실행
   * */
  useEffect(() => {
  ...
  if (!authData && authData === null && updateComponent) {
    return (
      <Suspense fallback={<Loading />}>
        <div
          className={`relative z-10 h-screen ${isFadeOut ? "hidden" : "flex"}`}
        ></div>
        {isAccess ? (
          <div
            className={`${
